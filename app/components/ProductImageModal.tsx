...
export function ProductImageModal({ isOpen, onClose, onSaveAsset }: ProductImageModalProps) {
  // Close on Escape key
  useEffect(() => {
...
function ProductImageModalContent({ 
  ...
  const [promptText, setPromptText] = useState(() =>
    buildProductPrompt({
      ...
      background: BACKGROUND_OPTIONS[0].name,
      lighting: LIGHTING_OPTIONS[0].name,
    })
  );

  // Sync prompt with selections unless manually overridden
  const syncPrompt = (product = selectedProduct, style = selectedStyle, bg = selectedBg, lighting = selectedLighting) => {
    const autoPrompt = buildProductPrompt({
  ...
  const handleProductChange = (prod: typeof PRODUCT_OPTIONS[0]) => {
    setSelectedProduct(prod);
    if (!isManualEdit) syncPrompt(prod, selectedStyle, selectedBg, selectedLighting);
  };

  const handleStyleChange = (style: typeof STYLE_OPTIONS[0]) => {
    setSelectedStyle(style);
    if (!isManualEdit) syncPrompt(selectedProduct, style, selectedBg, selectedLighting);
  };

  const handleBgChange = (bg: typeof BACKGROUND_OPTIONS[0]) => {
    setSelectedBg(bg);
    if (!isManualEdit) syncPrompt(selectedProduct, selectedStyle, bg, selectedLighting);
  };

  const handleLightingChange = (lighting: typeof LIGHTING_OPTIONS[0]) => {
    setSelectedLighting(lighting);
    if (!isManualEdit) syncPrompt(selectedProduct, selectedStyle, selectedBg, lighting);
  };

  const handleResetToTemplate = () => {
    syncPrompt(selectedProduct, selectedStyle, selectedBg, selectedLighting);
  };
  ...
  const runGeneration = async (isVariation = false) => {
    ...
    try {
      ...
      // Call generation service (underlying provider can be swapped at any time)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 md:p-6 overflow-y-auto">
      ...
      {/* Modal Window */}
      <div className="relative w-full max-w-6xl max-h-[92vh] overflow-hidden rounded-2xl border border-emerald-500/30 bg-slate-900/95 shadow-2xl shadow-emerald-950/40 backdrop-blur-2xl flex flex-col">
        {/* Top Glowing Ribbon */}
        ...
        <div className="flex items-center justify-between border-b border-white/[0.08] px-6 py-4 bg-slate-950/50">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 shadow-inner">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Configure attributes, review synthesized prompt directives, and render studio packshots.
              </p>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 p-6 space-y-6 overflow-y-auto">
                  ...
                  <button
                    ...
                    className={`flex flex-col text-left p-2.5 rounded-xl border transition-all ${
                      selectedProduct.id === prod.id
                        ? 'border-emerald-500 bg-emerald-500/15 text-white shadow-inner ring-1 ring-emerald-500/50'
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg border text-xs font-medium transition-all ${
                      selectedStyle.id === style.id
                        ? 'border-emerald-500 bg-emerald-500/20 text-white shadow-sm ring-1 ring-emerald-500/40'
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs font-bold uppercase tracking-wider text-emerald-400 flex items-center gap-1.5">
                  ...
                  <span>3. Background & Surface</span>
                </label>
                <span className="text-[11px] text-slate-400 font-mono">
                  {selectedBg.name.split('(')[0]}
                </span>
              </div>
                ...
                {BACKGROUND_OPTIONS.map((bg) => (
                  <button
                    ...
                    className={`flex items-center gap-2.5 p-2 rounded-xl border transition-all text-left ${
                      selectedBg.id === bg.id
                        ? 'border-emerald-500 bg-emerald-500/15 text-white ring-1 ring-emerald-500/50'
                  ...
                  >
                    <div 
                      ...
                      style={{ backgroundColor: bg.previewColor?.startsWith('#') ? bg.previewColor : '#334155' }}
                    />
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs font-bold uppercase tracking-wider text-emerald-400 flex items-center gap-1.5">
                  ...
                  <span>4. Studio Lighting</span>
                </label>
                <span className="text-[11px] text-slate-400 font-mono">
                  {selectedLighting.name.split('(')[0]}
                </span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            <div className="rounded-xl border border-white/[0.1] bg-slate-950/60 p-4 space-y-3">
              ...
              <textarea
                ...
                placeholder="Review or customize your generation prompt..."
                className="w-full rounded-xl border border-white/[0.1] bg-slate-900/90 p-3 text-xs text-slate-200 placeholder-slate-500 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500 leading-relaxed font-sans"
              />
            ...
            <div>
              <button
                ...
                className="w-full flex items-center justify-center gap-2.5 rounded-xl bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-500 py-3 px-6 text-sm font-bold text-white shadow-lg shadow-emerald-600/30 hover:shadow-emerald-600/50 hover:from-emerald-500 hover:to-teal-500 active:scale-[0.99] transition-all disabled:opacity-60 cursor-pointer"
              >
                {isGenerating ? (
                  <>
                  ...
                  <>
                    <Zap className="h-4 w-4 text-emerald-200 fill-emerald-200" />
          ...
          <div className="lg:col-span-5 p-6 bg-slate-950/40 flex flex-col justify-between space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between pb-3 border-b border-white/[0.06]">
                <div className="flex items-center gap-2">
                  ...
                  {generatedResult && (
                    <span className="rounded bg-emerald-500/15 border border-emerald-500/30 px-1.5 py-0.2 text-[10px] font-mono text-emerald-400 font-semibold">
            ...
            <div className="flex-1 flex items-center justify-center min-h-[300px]">
              {isGenerating ? (
                ...
                <div className="w-full flex flex-col items-center justify-center p-8 text-center space-y-4 rounded-2xl border border-emerald-500/20 bg-slate-900/60 backdrop-blur-md">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      {generationStep <= 1 && 'Pass 1/3: Computing product geometry & depth maps...'}
                      {generationStep === 2 && 'Pass 2/3: Simulating volumetric lighting & contact shadows...'}
                ...
                <div className="w-full flex flex-col items-center">
                  <div className="group relative w-full max-w-sm rounded-2xl border border-white/[0.15] overflow-hidden shadow-2xl bg-slate-950 transition-all duration-300 hover:border-emerald-500/40">
                    ...
                    <div className="border-t border-white/[0.08] bg-slate-950/90 px-4 py-2.5 flex items-center justify-between text-[11px] font-mono text-slate-400">
                      ...
                      <span className="text-emerald-400 font-semibold">{generatedResult.resolution}</span>
                    </div>
                  </div>
                </div>
              ) : (
                ...
                <div className="w-full flex flex-col items-center justify-center p-8 text-center rounded-2xl border border-dashed border-white/[0.1] bg-slate-900/30">
                  ...
                  <p className="text-[11px] text-slate-400 mt-1 max-w-xs leading-relaxed">
                    Select your product, style, lighting, and surface options, review the generated prompt, and click <strong className="text-emerald-400">Generate Product Image</strong>.
                  </p>
                  <div className="mt-4 flex items-center gap-1.5 text-[10px] text-slate-500">
                    <Clock className="h-3 w-3" />
                    <span>Latency: ~1.5s (Mock Provider)</span>
                  </div>
                </div>
              )}
            </div>
              ...
              <div className="space-y-3 pt-4 border-t border-white/[0.08]">
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <button
                    ...
                    title="Generate an alternate variation angle"
                  >
                  ...
                  >
                    {isSaved ? <Check className="h-3.5 w-3.5 text-emerald-400" /> : <Bookmark className="h-3.5 w-3.5 text-slate-400" />}
            ...
            <div className="pt-2 text-center text-[10px] text-slate-400">
              Internal AI Asset Studio • {generatedResult?.meta.renderEngine || 'Product Packshot Subsystem v2.4'}
            </div>
